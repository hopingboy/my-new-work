...
# Production-ready installer for the HKVM executable
# ============================================================
...
cleanup() {
    rm -f "${INSTALL_DIR}/hkvm.download" 2>/dev/null || true
}
...
if command -v systemctl >/dev/null 2>&1 &&
   [[ -d /run/systemd/system ]]; then
    ...
    warn "HKVM will be installed without a systemd service."
fi
...
info "Downloading HKVM executable..."

TEMP_FILE="${INSTALL_DIR}/hkvm.download"
...
if [[ ! -f "${TEMP_FILE}" ]]; then
    die "HKVM download failed."
fi

if [[ ! -s "${TEMP_FILE}" ]]; then
    die "Downloaded HKVM file is empty."
fi
...
if echo "${FILE_TYPE}" | grep -Eiq \
    ...
    die "Invalid HKVM executable received."
fi
...
info "Installing HKVM executable..."
...
if [[ ! -x "${BIN_FILE}" ]]; then
    die "Unable to make HKVM executable."
fi
...
if command -v ldd >/dev/null 2>&1; then
    ...
    if echo "${LDD_OUTPUT}" | grep -q "not a dynamic executable"; then
        info "HKVM is statically linked."
    elif echo "${LDD_OUTPUT}" | grep -q "not found"; then
        warn "HKVM may have missing shared libraries:"
        echo "${LDD_OUTPUT}"
    fi
fi
...
EOF
    ...
    if kill -0 "${HKVM_PID}" 2>/dev/null; then
        ok "HKVM started successfully. PID: ${HKVM_PID}"
    else
        die "HKVM failed to start. Check ${LOG_FILE}"
    fi
fi
